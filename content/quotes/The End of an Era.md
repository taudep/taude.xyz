---
author: Hugh Howey
URL:
title: Quotes from The End of an Era
source: https://hughhowey.com/the-end-of-an-era/
date: 2026-07-31
original_date: ""
draft: false
tags: []
slug: "the-end-of-an-era"
---
Hugh Howey had some insights about the publishing industry and the future of being a writer in the age of AI.  Some highlights:

"One prompt, and all this spills out. And if your goal is a blog post, you’re gonna love this era. If your goal is the joy of thinking in words, nothing has changed. So this is the real part, right here: Doing a thing for the love of the thing is no different than it ever was. The problem is, there aren’t many people for whom this applies''


"It’s never been easier to be an artist. It’s also never been easier to make money PRETENDING to be an artist. Which means is’t never been more challenging and confusing to be an artist whose goal is to make a living." 

The whole article is worth a read.  There's a whole discussion to be had on using an LLM to write the story, vs having an LLM assist with writing a story (correcting sentences, helping with plotting brainstorming, character names, etc.)